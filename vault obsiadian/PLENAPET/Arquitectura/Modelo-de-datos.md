...
- **lab_results**: `id, lab_panel_id, analyte_key, exam_type, analyte_name, value, value_text, unit, reference_min, reference_max, organ_system, created_at` — un analito individual, cargado por el equipo interno desde `/admin/salud`, normalmente eligiéndolo del catálogo estándar (`packages/database/src/exam-catalog.ts`, migración `0004`) en vez de texto libre. `analyte_key` (ej. `"creatinina"`, `"sdma"`) permite lógica confiable sobre analitos específicos, como la estadificación IRIS (`getIrisStage()` en `health-scoring.ts`) — es `null` cuando el analito se cargó como "otro" (fuera del catálogo). `reference_min/max` los define el laboratorio aliado en su reporte real; el catálogo solo sugiere un valor de partida.
