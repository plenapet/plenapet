...
- **Aplicar la migración `supabase/migrations/0004_lab_results_analyte_key.sql`** en el SQL Editor de Supabase — sin esto, el estadio IRIS (renal) no se puede calcular porque no hay forma confiable de identificar cuál resultado cargado es la creatinina/SDMA.
- **Probar el flujo completo de PlenaPet Health de punta a punta** (nadie lo ha probado en el navegador todavía): registrar una cuenta de cliente en `/cuenta/registro` → agregar una mascota en `/health/mascotas/nueva` → completar la encuesta de bienestar → como admin, ir a `/admin/salud`, buscar esa mascota, crear un panel de laboratorio, cargar creatinina/SDMA desde el catálogo (para ver el estadio IRIS) y publicarlo → volver a `/health/mascotas/[id]` como cliente y confirmar que el dashboard muestra el puntaje por sistema, el estadio IRIS y la edad biológica estimada correctamente.
- **Conseguir el paper completo de Banzato et al. 2019 (Canine Frailty Index)** para replicar fielmente los 33 ítems publicados en la encuesta de bienestar, en vez de los 16 propios usados como primera versión — ver [[Vitalidad-y-Longevidad]].
